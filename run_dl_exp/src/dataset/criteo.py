...
class CriteoDataset(torch.utils.data.Dataset):
    ...
    def __get_feat_mapper(self, path):
        feat_cnts = defaultdict(lambda: defaultdict(int))  # {field_id, feature, cnt}
        with open(path) as f:
        ...
        feat_mapper = {i: {feat for feat, c in cnt.items() if c >= self.min_threshold} for i, cnt in feat_cnts.items()}  # {field_id, feature_set} 
        feat_mapper = {i: {feat: idx for idx, feat in enumerate(cnt)} for i, cnt in feat_mapper.items()}  # {field_id, feature_id, feature}
        defaults = {i: len(cnt) for i, cnt in feat_mapper.items()}  # {field_id, field_num}
