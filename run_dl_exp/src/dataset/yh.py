...
class YHDataset(Dataset):
    def __init__(self, dataset_path=None, data_prefix='tr', rebuild_cache=False, tr_max_dim=-1, read_flag='0'):
        ...
        with self.env.begin(write=False) as txn:
            ...
            self.pos_cum_sum = np.cumsum(np.frombuffer(txn.get(b'pos_num'), dtype=np.int32))
            ...
            print('Totally %d contexts, %d items, %d dims, %d samples'%(len(self.pos_cum_sum), self.item_num, self.max_dim, self.length))
    ...
    def __build_cache(self, data_path, item_path, cache_path):
        ...
        if ctx_col.returncode or item_col.returncode:
            ...
            self.max_ctx_num = int(ctx_col.stdout.strip()) - 1  # -1: reomve labels
        ...
        with lmdb.open(cache_path, map_size=int(1e11)) as env:
            i = 0
            items = np.zeros((2000, self.max_item_num), dtype=np.int32)
            ...
            for buf in self.__yield_buffer(data_path):
                with env.begin(write=True) as txn:
                    for item_key, item_array, ctx_key, ctx_array, max_dim_buf, pos_num_buf in buf:
            ...
            with env.begin(write=True) as txn:
                ...
                txn.put(b'pos_num', np.array(pos_num, dtype=np.int32).tobytes())
    ...
    def __yield_buffer(self, data_path, buffer_size=int(1e6)):
        sample_idx, max_dim, pos_num = 0, 0, []
        buf = list()
        with open(data_path, 'r') as fd:
            ...
            for line in pbar:
                ...
                item_array = np.zeros((2, pos_num[-1]), dtype=np.float32)
...
if __name__ == '__main__':
    ...
    dataset = YHDataset(dataset_path=sys.argv[1], data_prefix='det_trva', rebuild_cache=1, tr_max_dim=-1, read_flag='1')
    print(len(dataset))
    ...
    data_loader = DataLoader(dataset, batch_size=24, num_workers=0, shuffle=False) #worker_init_fn=worker_init_fn, sampler=srs(sample_list))
    pbar = tqdm(data_loader, smoothing=0, mininterval=1.0, ncols=100)
